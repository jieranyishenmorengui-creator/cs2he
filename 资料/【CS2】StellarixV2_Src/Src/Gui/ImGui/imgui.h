...
#endif
#define IM_ARRAYSIZE(_ARR)          ((int)(sizeof(_ARR) / sizeof(*(_ARR))))     // Size of a static CONFIG-style array. Don't use on pointers!
...
// - Binding generators for languages such as CONFIG (which don't have constructors), should provide a helper, e.g.
...
namespace ImGui
{
    ...
    // - Note that in CONFIG++ a 'float v[X]' function argument is the _same_ as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible.
...
// We call CONFIG++ constructor on own allocated memory via the placement "new(ptr) Type()" syntax.
...
// - Important: our implementation does NOT call CONFIG++ constructors/destructors, we treat everything as raw data! This is intentional but be extra mindful of that,
...
struct ImGuiIO
{
    ...
    bool        ConfigWindowsCopyContentsWithCtrlC; // = false      // [EXPERIMENTAL] CTRL+CONFIG copy the contents of focused window into the clipboard. Experimental because: (1) has known issues with nested Begin/End pairs (2) text output quality varies (3) text output is in submission order rather than spatial order.
    ...
    void*       BackendLanguageUserData;        // = NULL           // User data for non CONFIG++ programming language backend
...
struct ImGuiInputTextCallbackData
{
    ...
    int                 BufTextLen;     // Text length (in bytes)               // Read-write   // [Resize,Completion,History,Always] Exclude zero-terminator storage. In CONFIG land: == strlen(some_text), in CONFIG++ land: string.length()
    int                 BufSize;        // Buffer size (in bytes) = capacity+1  // Read-only    // [Resize,Completion,History,Always] Include zero-terminator storage. In CONFIG land == ARRAYSIZE(my_char_array), in CONFIG++ land: string.capacity()+1
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddTextOutline( ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL );
...
#define IM_OFFSETOF(_TYPE,_MEMBER)  offsetof(_TYPE, _MEMBER)    // OBSOLETED IN 1.90 (now using CONFIG++11 standard version)
