...
namespace CONFIG::BIN
{
	/* @section: [internal] */
	/// write single variable to the buffer
	/// @returns: number of written bytes
	__forceinline std::size_t WriteBuffer( std::uint8_t* pBuffer, const VariableObject_t& variable )
	{
	...
	__forceinline std::size_t ReadBuffer( std::uint8_t* pBuffer, VariableObject_t& variable )
	{
		std::uint8_t* pBufferCurrent = pBuffer;
	...
	__forceinline std::uint8_t* FindBuffer( const std::uint8_t* pBufferStart, const std::size_t nBufferSize, const VariableObject_t& variable )
	{
		...
		return nullptr;
#else
		return GetMemoryManager()->FindPatternEx( pBufferStart, nBufferSize, reinterpret_cast< const std::uint8_t* >( uNameTypeHash ), sizeof( FNV1A_t[ 2 ] ) );
#endif
	}

	/* @section: main */
	__forceinline bool SaveVariable( const wchar_t* wszFilePath, const VariableObject_t& variable )
	{
		...
		std::uint8_t* pBuffer = static_cast< std::uint8_t* >( GetMemoryManager( )->HeapAlloc( dwFileSize ) );
		if ( !::ReadFile( hFileInOut, pBuffer, dwFileSize, nullptr, nullptr ) )
		{
			::CloseHandle( hFileInOut );

			GetMemoryManager( )->HeapFree( pBuffer );
			return false;
		}
	...
	__forceinline bool LoadVariable( const wchar_t* wszFilePath, VariableObject_t& variable )
	{
		const HANDLE hFileInOut = ::CreateFileW( wszFilePath, GENERIC_WRITE | GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr );
		if ( hFileInOut == INVALID_HANDLE_VALUE )
			return false;
	...
	__forceinline bool RemoveVariable( const wchar_t* wszFilePath, const VariableObject_t& variable )
	{
		const HANDLE hFileInOut = ::CreateFileW( wszFilePath, GENERIC_WRITE | GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr );
		if ( hFileInOut == INVALID_HANDLE_VALUE )
			return false;
	...
	__forceinline bool SaveFile( const wchar_t* wszFilePath )
	{
		...
		std::uint8_t* pBuffer = static_cast< std::uint8_t* >( GetMemoryManager( )->HeapAlloc( nBufferSize ) );
	...
	__forceinline bool LoadFile( const wchar_t* wszFilePath )
	{
		const HANDLE hFileInOut = ::CreateFileW( wszFilePath, GENERIC_WRITE | GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr );
		if ( hFileInOut == INVALID_HANDLE_VALUE )
			return false;
