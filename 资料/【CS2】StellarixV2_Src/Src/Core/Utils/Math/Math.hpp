...
namespace MATH
{
	...
	auto AngleVectors( Vector_t angles, Vector_t& forward, Vector_t& right, Vector_t& up ) -> void;


	auto CalculateCameraPosition( Vector_t anchorPos, float distance, QAngle_t viewAngles ) -> Vector_t;
	auto NormalizeAngles( QAngle_t angles ) -> QAngle_t;
	auto CalcAngle( Vector_t viewPos, Vector_t aimPos ) ->QAngle_t;
}