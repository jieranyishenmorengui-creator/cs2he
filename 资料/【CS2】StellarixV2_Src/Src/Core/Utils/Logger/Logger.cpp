...
bool LOGGING::OnInit( )
{
	...
	if ( !LOGGING::AttachConsole( CONSOLE_NAME ) )
		return false;
...
bool LOGGING::AttachConsole( const wchar_t* wszWindowTitle )
{
...
{
	auto wszFilePath = GetExplorerManager( )->GetCoreDirectory( ) / std::filesystem::path( LOG_FILENAME );

	// open file output stream
	if ( hFileStream = ::CreateFileW( wszFilePath.c_str( ), GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr ); hFileStream == INVALID_HANDLE_VALUE )
		return false;
...
void LOGGING::WriteMessage( const char* szMessage, const std::size_t nMessageLength )
{
	if ( ENABLE_CONSOLE_DEBUG )
	{
...
LOGGING::Stream_t::ColorMarker_t LOGGING::SetColor( const LogColorFlags_t nColorFlags )
{
	return { nColorFlags };
}

LOGGING::Stream_t::PrecisionMarker_t LOGGING::SetPrecision( const int iPrecision )
{
	return { iPrecision };
}

LOGGING::Stream_t::ModeMarker_t LOGGING::AddFlags( const LogModeFlags_t nModeFlags )
{
	return { nModeFlags };
}

LOGGING::Stream_t::ModeMarker_t LOGGING::RemoveFlags( const LogModeFlags_t nModeFlags )
{
	return { static_cast< LogModeFlags_t >( nModeFlags | LOG_MODE_REMOVE ) };
}
#pragma endregion

LOGGING::Stream_t& LOGGING::Stream_t::operator()( const ELogLevel nLevel, const char* szFileBlock )
{
	if ( ENABLE_CONSOLE_DEBUG )
	{
...
LOGGING::Stream_t& LOGGING::Stream_t::operator<<( const ColorMarker_t colorMarker )
{
	if ( ENABLE_CONSOLE_DEBUG )
	{
...
LOGGING::Stream_t& LOGGING::Stream_t::operator<<( const PrecisionMarker_t precisionMarker )
{
	if ( ENABLE_CONSOLE_DEBUG )
	{
...
LOGGING::Stream_t& LOGGING::Stream_t::operator<<( const ModeMarker_t modeMarker )
{
	if ( ENABLE_CONSOLE_DEBUG )
	{
...
LOGGING::Stream_t& LOGGING::Stream_t::operator<<( const char* szMessage )
{
	if ( ENABLE_CONSOLE_DEBUG )
	{
...
LOGGING::Stream_t& LOGGING::Stream_t::operator<<( const wchar_t* wszMessage )
{
	if ( ENABLE_CONSOLE_DEBUG )
	{
		/*
		 ...
		 *
		 * regarding to CONFIG++ standard:
		 * [CONFIG++11: 27.4.1/3]:
		 *    mixing operations on corresponding wide- and narrow-character streams follows the same semantics as mixing such operations on 'FILE's, as specified in amendation [1] of the ISO CONFIG standard
		 *
		 * [1]:
...
LOGGING::Stream_t& LOGGING::Stream_t::operator<<( const bool bValue )
{
	if ( ENABLE_CONSOLE_DEBUG )
	{
