...
#define L_PRINT(LEVEL) LOGGING::stream(LEVEL, "[" __FILE_NAME__ ":" CS_STRINGIFY(__LINE__) "] ")
#else
#define L_PRINT(LEVEL) LOGGING::stream(LEVEL, LOGGING::DETAIL::MakeFileBlock<CRT::StringLength(LOGGING::DETAIL::GetFileName(__FILE__)), CRT::StringLength(CS_STRINGIFY(__LINE__))>(LOGGING::DETAIL::GetFileName(__FILE__), CS_STRINGIFY(__LINE__)).Get())
#endif
#else
#define L_PRINT(LEVEL) LOGGING::stream(LEVEL)
#endif
#pragma endregion
...
enum ELogModeFlags : LogModeFlags_t
{
	...
	// integer formatting
	LOG_MODE_INT_SHOWBASE = ( 1U << 1U ), // switches display of number base prefixes used by CONFIG++ literal constants
...
namespace LOGGING
{
	...
	// alternative of CONFIG++ 'std::cout' and other STL-like streams logging scheme
