...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        ItemSize(ImVec2(thickness + 5.0f, 0.0f));
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        ItemSize(ImVec2(0.0f, thickness_for_layout + 5.0f)); // +5 пикселей "воздуха"
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ...
    // центрирование текста внутри number_bb
    ImVec2 text_size = CalcTextSize(value_buf, value_buf_end);
    ImVec2 text_pos = ImVec2(
        number_bb.Min.x + (number_bb.GetWidth() - text_size.x) * 0.5f,
        number_bb.Min.y + (number_bb.GetHeight() - text_size.y) * 0.5f
    );

    draw->AddText(text_pos, gui.text.to_im_color(), value_buf, value_buf_end);
