...
#include "../../utils/typedefs/c_utl_map.h"
...
public:
	...
	char* world_clr_display_model; // 0xe8
	char* m_holstered_model; // 0xf0
	char* world_clr_extra_wearable_model; // 0xf8
...
{
public:
	virtual c_econ_item_schema* get_econ_item_schema() = 0;
};

class c_debug_overlay_system
{
public:
	void add_box_overlay(const vec3_t& end, const vec3_t& mins, const vec3_t& maxs, const vec3_t& a3, c_color_rgba clr)
	{
		using function_t = void(__thiscall*)(c_debug_overlay_system*, const vec3_t&, const vec3_t&, const vec3_t&, const vec3_t&, int, int, int, int, double);
		(*reinterpret_cast<function_t**>(std::uintptr_t(this)))[48](this, end, mins, maxs, a3, clr.r, clr.g, clr.b, clr.a, static_cast<double>(4.f));
	}

	void add_box_overlay2(const vec3_t& origin, const vec3_t& mins, const vec3_t& maxs, const vec3_t& rotation, const c_color_rgba& face_color, const c_color_rgba& edge_color, float duration)
	{
		using function_t = void(__thiscall*)(c_debug_overlay_system*, const vec3_t&, const vec3_t&, const vec3_t&, const vec3_t&, const c_color_rgba&, const c_color_rgba&, double);
		(*reinterpret_cast<function_t**>(std::uintptr_t(this)))[48](this, origin, mins, maxs, rotation, face_color, edge_color, static_cast<double>(duration));
	}
};
...
public:
	c_econ_item_system* get_econ_item_system() {
		return vmt::call_virtual<c_econ_item_system*>(this, 122);
	}

	c_debug_overlay_system* get_debug_overlay_system()
	{
		return vmt::call_virtual<c_debug_overlay_system*>(this, 166);
	}
};