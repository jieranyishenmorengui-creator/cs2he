...
static constexpr int UPDATE_INTERVAL_MS = 250; // 每秒扫4次, 不拖帧
    ...
    for (int i = 1; i < 64; ++i) {
        ...
        if (target_pawn == local_pawn) {
            ...
            if (name[0])
                ...
                char tag[32];
                snprintf(tag, sizeof(tag), "Player #%d", i);
                s_spectators.emplace_back(tag);
            }
        }
    }
}
